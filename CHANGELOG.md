...
- **`From<(u8, u8, u8)> for Rgb888`** and **`From<Rgb888> for (u8, u8, u8)`**, bringing the bare
  tuple to parity with the existing `[u8; 3]` conversions. `rgb::distance_sq` and `Rgb::to_rgb`
  both standardize on `(u8, u8, u8)` as gem's raw-channel currency, but without these impls an API
  written as `impl Into<(u8, u8, u8)>` accepted bare tuples and rejected `Rgb888`, with no bound
  that admitted both. Deliberately not added for `Bgr888`: `Bgr::to_bgr` returns memory order and
  `Bgr::to_rgb` returns color order, so a bare `From<Bgr888> for (u8, u8, u8)` would be ambiguous
  about which one it means; `Bgr888` keeps its explicit named methods instead.
